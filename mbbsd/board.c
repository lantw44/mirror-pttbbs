...
load_boards(char *key)
{
	...
	if( yank_flag == 0 ){ // fav mode
		...
		if (is_set_attr(&fav->favh[i], FAVH_ADM_TAG))
		    state |= PBS_TAG;
		addnewbrdstat(fav_getid(&fav->favh[i]) - 1, PBS_FAV | state);
	    }
	    if (brdnum == 0)
		addnewbrdstat(0, 0);
	    byMALLOC = 0;
	...
	while (++myrow < b_lines) {
		...
		if (ptr->myattr & PBS_LINE){
			...
			prints("%5d %c %s------------      ------------------------------------------\033[m",
				...
				ptr->myattr & PBS_FAV ? "" : "\033[1;30m");
		    else
			prints("        %s------------      ------------------------------------------\033[m", ptr->myattr & PBS_FAV ? "" : "\033[1;30m");
		    continue;
		}
		else if (ptr->myattr & PBS_FOLDER){
		...
		if (!newflag) {
		    prints("%5d%c%s", head,
			   ...
			   unread[ptr->myattr & PBS_UNREAD ? 1 : 0]);
		} else {
		    if (B_BH(ptr)->brdattr & BRD_GROUPBOARD)
			prints("        ");
		    else
...
choose_board(int newflag)
{
	...
	case Ctrl('L'):
		...
		tmp = generalnamecomplete("½Ð¿é¤J¬ÝªO­^¤å¦WºÙ¡G",
                            buf, sizeof(buf),
                            SHM->Bnumber,
	...
	case 'Z':
	    if (HAS_PERM(PERM_BASIC)) {
		sprintf(buf, "½T©w­n %s­q¾\\ ·s¬ÝªO? [N/y] ", cuser.uflag2 & FAVNEW_FLAG ? "¨ú®ø" : "");
		if (getans(buf) != 'y')
		    break;
